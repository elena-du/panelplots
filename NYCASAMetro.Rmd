...
## Facet by gear
#labeling_both is about factor and cat variable
```{r}
g + facet_grid(gear ~ ., 
               labeller = label_both)
```
...
# i deleted labeller = label_both, the graph didnt change so it is just unused argument
```{r}
g + facet_grid(gear ~ ., scales = "free_y", 
               space = "free_y")
```
...
ggplot(aes(log10(budget), length)) + geom_point() +
  facet_wrap(~Action) 
```
